...
05 Feb 2008 -  NMD
    buildasn.py and updateasn.py were moved to stsdas$python
temporarily until the refactoring is finished. They are used only by
tweak.py and there's no name colision with the new tasks.
...
from stsci.tools.fileutil import buildRootname
from stsci.tools import asnutil #asnutil import readShiftFile
...
__version__ = '0.9.5b (28-June-2005)'
...
def buildAsnTable(asnroot,suffix=None,shiftfile=None,verbose='no'):
    ...
    if shiftfile != None:
        ...
        shiftdict = asnutil.ShiftFile(shiftfile)
...
def _buildAsnPrimary(fasn,output,img1,frame=None,refimage=None):
    ...
    """
    origin_str = 'Astropy Version '+astropy.__version__
