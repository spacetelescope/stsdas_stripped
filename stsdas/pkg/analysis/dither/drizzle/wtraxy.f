...
C
C V2.5 bug fix in SIP support (was trying to read SIP header from wrong file)
C       Richard Hook, ST-ECF, October 2008.
C
C--
      IMPLICIT NONE
...
C-- Start of executable code
      VERS='WTRAXY Version 2.5.1 (1st May 2009)'
...
C
C A bug occured here in earlier versions when coeffs="wcs" - the SIP coefficients were read from the
C output rather than the input. 
C
C Richard Hook, ST-ECF, October 2008
...
C         IF(NINT(XIN).GT.1 .AND. NINT(XIN).LT.NXIN-1 .AND.
C     :         NINT(YIN).GT.1 .AND. NINT(YIN).LT.NYIN-1) THEN
...
C         ELSE 
C            CALL DRIVAL(XIN,YIN,1,NXIN,NYIN,NXOUT,NYOUT,.FALSE.,
C     :            XSH,YSH,ROT,SCALE,ALIGN,ROTFIR,
C     :            SECPAR,XSH2,YSH2,ROT2,XSCALE,YSCALE,SHFR2,ROTF2,
C     :            USEWCS,WCSIN,WCSOUT,
C     :            COTY,CONUM,XCO,YCO,
C     :            .FALSE.,MEMR(PXG),MEMR(PYG),XGDIM,YGDIM,XOUT,YOUT)
C         ENDIF
...
C Write out the result
         IF(.NOT.SILENT) THEN
            WRITE(CHARS,
     :  '('' Xin,Yin: '',2F13.6'' Xout,Yout: '',2F13.6,
     :    '' PAM: '',F12.5)')
