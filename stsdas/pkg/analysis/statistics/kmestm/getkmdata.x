...
# GETKMDATA -- Subroutine get_km_data adapted from the ASURV routine datain
# AND with apologies to the columns task in IRAF. 
# This routine reads data from the input file for the Kaplan-Meier univariate 
# estimator problem. The input file may be either an ascii text or a table file.

int procedure get_km_data (filename, section, ind, x, ntot)
...
begin
	...
	if (not_table) {
	    ...
	    # Skip commented out(#) lines or null lines
		    ...
		    # Check for INDEF's
		    if (ind[1,ntot] == INDEFI || x[1,ntot] == INDEFD) {
