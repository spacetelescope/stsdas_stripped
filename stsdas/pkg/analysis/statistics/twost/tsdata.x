...
#* B.Simon	01-Sep-92	revised version of get_ts_data

# TSDATA -- Data input routine for km estimator task

int procedure tsdata (tp, section, ind, ista, x, ntot)

pointer	tp		# i: input table descriptor
char	section[ARB]	# i: table section (contains column names)
int	ind[ARB]	# o: censor indicator
int	ista[ARB]	# o: which group does this point belong to
...
begin
	...
	# Read column containing group number
	...
	if (cp == NULL) {
	    ...
	    call tbcgti (tp, cp, ista, Memb[nullflag], 1, ntot)
