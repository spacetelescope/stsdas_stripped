...
# DO_PETO -- Do Peto & Prentice generalized Wilcoxon test 

procedure do_peto (verbose, isign, id1, id2, xy, n1, n2, ncomp, ntot)
...
pointer	sp, d, e, r, d1, e1, r1, d2, e2, r2, f, a, ft, at, xa

string	title  "\n%7w Peto & Prentice Generalized Wilcoxon Test \n\n"
...
begin
	...
	call salloc (a, ntot, TY_DOUBLE)
	call salloc (ft, ntot, TY_DOUBLE)
	call salloc (at, ntot, TY_DOUBLE)
	call salloc (xa, ntot, TY_DOUBLE)

	call printf (title)

	# Calculate test results

	call peto (id1, id2, xy, n1, n2, ncomp, ntot, test, prob,
		   Memd[d], Memd[e], Memd[r], Memd[d1], Memd[e1],
		   Memd[r1], Memd[d2], Memd[e2], Memd[r2], Memd[f],  
		   Memd[a], Memd[ft], Memd[at], Memd[xa], score, var)
	...
	if (verbose) {
		...
		temp1 = isign * xy[i]
		temp2 = (2.0 * Memd[f+j] - 1.0) * Memd[d2+j] +
			(Memd[f+j] - 1.0) * Memd[e2+j]
