...
class DrizzleObject(object):
    ...
    def _make_WCS_header(self):
        ...
        for index in range(1, len(fits_output)):
            ...
            fits_output[index].header.set('CDELT2', WCS_input['CDSCALE'], '[arcsec/pixel] cross-dispersion scale', after='DATE')
            fits_output[index].header.set('CRVAL2',  0.0, '[arcsec] reference value', after='DATE')
            fits_output[index].header.set('CRPIX2', WCS_input['YOFFS'], '[pix] reference pixel', after='DATE')
            fits_output[index].header.set('CUNIT2', 'arcsec', 'cross-dispersion units', after='DATE')
            fits_output[index].header.set('CTYPE2', 'CRDIST', 'cross-dispersion distance', after='DATE')

            fits_output[index].header.set('CDELT1', WCS_input['DLAMBDA'], '[Angstrom/pixel] dispersion', after='DATE')
            fits_output[index].header.set('CRVAL1', WCS_input['LAMBDA0'], '[Angstrom] reference value', after='DATE')
            fits_output[index].header.set('CRPIX1', WCS_input['XOFFS'], '[pixel] reference pixel', after='DATE')
            fits_output[index].header.set('CUNIT1', 'Angstrom', 'dispersion units', after='DATE')
            fits_output[index].header.set('CTYPE1', 'WAVE', 'grating dispersion function', after='DATE')
        ...
        if 'NUM_DRIZ' in header:
            ...
            for index in range(num_driz):
                ...
                if img_kword in header and header[img_kword] in reject_info:
                    ...
                    header.set(kword1, kval1, comment1, after=img_kword)
                    ...
                    header.set(kword2, float(kval2), comment2, after=kword1)
