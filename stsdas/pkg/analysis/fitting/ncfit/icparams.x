...
# Thuis file contains procedures for opening, closing and accessing
...
begin
	...
	call ic_puti (ic, "pcomp", NO)
	call ic_puti (ic, "xaxis", IC_LINEAR)
	call ic_puti (ic, "yaxis", IC_LINEAR)
	call ic_puti (ic, "ltype", IC_CONTINUOUS)
...
begin
	...
	IC_XAXIS(icout) = IC_XAXIS(icin)
	IC_YAXIS(icout) = IC_YAXIS(icin)
...
# IC_CLOSE -- Close ICFIT parameter structure.

procedure ic_close (ic)
...
begin
	if (streq (param, "sample"))
	    ...
	    call malloc (ptr, SZ_LINE, TY_CHAR)
	    i = strdic (str, Memc[ptr], SZ_LINE, METHODS)
	    if (i > 0)
		IC_METHOD(ic) = i
	    call mfree (ptr, TY_CHAR)
...
begin
	if (streq (param, "naverage"))
	    ...
	    IC_GKEY(ic) = ival
	else if (streq (param, "pcomp"))
	    IC_COMP(ic) = ival
	else if (streq (param, "xaxis"))
	    IC_XAXIS(ic) = ival
	else if (streq (param, "yaxis"))
	    IC_YAXIS(ic) = ival
	else if (streq (param, "ltype"))
	    IC_LTYPE(ic) = ival
	else
	    call error (0, "ICFIT: Unknown parameter")
end
...
begin
	if (streq (param, "sample"))
	    ...
	    call ic_dent (FUNCTIONS, IC_FUNCTION(ic), str, maxchars)
	else if (streq (param, "algorithm"))
	    call ic_dent (METHODS, IC_METHOD(ic), str, maxchars)
	else
	    call error (0, "ICFIT: Unknown parameter")
end


# IC_DENT -- Get a dictionary entry, given the index. The string separation
# character in the dictionary is assumed to be its first character (same
# format as used in strdic). Actually, this routine performs the inverse
# operation found in iraf$sys/fmtio/strdic.x.

procedure ic_dent (dic, index, outstr, maxchars)

char	dic[ARB]		#i: dictionary string
int	index			#i: index of string to get
char	outstr[ARB]		#o: output string
int	maxchars		#i: max. size of output string

int	i, j

begin
	if ((dic[1] == EOS) || (index == 0)) {
	    outstr[1] = EOS
	    return
	}
