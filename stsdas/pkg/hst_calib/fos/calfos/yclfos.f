...
*  Description:
*  ------------
*       This routine performs the calibration of FOS data.  It has
*       two input cl parameters:
...
* 3.3       Aug 2000    M. De La Pena   Modified version and added msg regarding
*                                       the STECF POA_CALFOS package.
* 3.4       Dec 2001    M. De La Pena   Incremented version due to additional
*                                       checks added in YPFLAG to ensure no
*                                       component of flux calibration is done
*                                       when WAV_CORR is OMIT; fix by STPOA.
*                                       Changed PFLAGS values to SKIPPED from
*                                       OMIT when appropriate. 
* 3.5       Feb 2002    M. De La Pena   Incremented version due to check in
*                                       yclprc.f to determine if poa_calfos
*                                       should be used on the data for the 
*                                       best calibration.  Processing via
*                                       calfos continues regardless.
* 3.6       Mar 2002    M. De La Pena   Added message regarding possible 
*                                       mismatch of calibration files btwn those
*                                       used by CALFOS vs POA_CALFOS.
* 3.7       Mar 2002    M. De La Pena   Removed poa_calfos messages.  Merged
*                                       with messages in stcomply.f.
*-------------------------------------------------------------------------------
C
C     Version number
C
...
C
        IF(GRNDMD.NE.'TIME-TAGGED')THEN
                CALL YCLPRC(ROOT,ROOTO,GRNDMD,ISTAT)
          ELSE
