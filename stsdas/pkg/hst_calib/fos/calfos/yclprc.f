...
*      12       Jan 02  M. De La Pena   Set abs flux calibrations to SKIPPED
*                                       (not OMIT) when no WAV_CORR performed.
*                                       Added checks to old flux calib method
*                                       to SKIP also if no WAV_CORR performed.
*      13       Feb 02  M. De La Pena   Added stcomply.f to check if the
*                                       dataset should be processed by
*                                       poa_calfos for the best calibration.
*-------------------------------------------------------------------------------
*
* INPUTS:
...
C
        ...
        IF(ISTAT.NE.0) GO TO 999
C
C CHECK if data should be processed by poa_calfos for the best calibration.
C If so, issue a warning message, but continue processing.
C
        CALL STCOMPLY(GRNDMD, ISTAT)
        IF(ISTAT.NE.0) GO TO 999
C
...
C
C
C allocate memory for XOFFS and YOFFS (GIMP correction)
C
...
C
     ...
     $                      DESCRP(18),DESCRP(19),ISTAT)
                    IF(ISTAT.NE.0)GO TO 999
		    IF(PEDGRE(18)(1:5).EQ.'DUMMY') PFLAGS(2)='SKIPPED'
        	ENDIF
C
...
C
     ...
     $                          PEDGRE(20),DESCRP(1),DESCRP(20),ISTAT)
                    IF(ISTAT.NE.0)GO TO 999
