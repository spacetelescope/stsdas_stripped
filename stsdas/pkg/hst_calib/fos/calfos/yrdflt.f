        SUBROUTINE YRDFLT(KEYWRD,NAME,MAXLEN,FOUND,FF,
     *                    PEDGRE,DESCRP,ISTAT)
*
*  Module number:
*
...
*  Keyphrase:
*  ----------
*       read flat field reference file
*  Description:
*  ------------
*       This routine reads the flat field reference file.
...
*
*  FORTRAN name: yrdflt.for
*
*  Keywords of accessed files and tables:
*  --------------------------------------
*  Name                        I/O     Description / Comments
*       NAME                    I       flat field reference file
*
*  Subroutines Called:
*  -------------------
*  CDBS:
...
*       ff - MAXLEN x 2 array containing flat fields for one or both apertures.
...
C
        ...
        IF(ISTAT.NE.0)THEN
            CONTXT='ERROR opening flat field file '//KEYWRD//NAME
            GO TO 999
        ENDIF
C
...
C
        ...
        IF(ISTAT.NE.0)THEN
            CONTXT='ERROR reading flat field file '//NAME
            GO TO 998
        ENDIF
        IF(NAXIS.NE.1) THEN
            CONTXT='ERROR: flat field file has '//
     *              'invalid dimensions '//NAME
            GO TO 998
        ENDIF
            IF(DIMEN(1).GT.MAXLEN)THEN
                CONTXT='ERROR: flat field reference file to large '
                GO TO 998
            ENDIF
C
...
C
     ...
     *              'flat field file does not match observation'
                    GO TO 998
            ENDIF
        ENDIF
C
...
C
        ...
        IF(PAIRED)THEN
            ...
            IF(ISTAT.NE.0)THEN
                CONTXT='ERROR: APER_POS keyword missing from flat'//
     *                  ' field file'
                GO TO 999
            ENDIF
            ...
            IF(IPOS.EQ.0)THEN
                CONTXT='ERROR: invalid APER_POS in flat field file'
                GO TO 999
            ENDIF
        ENDIF
C
...
C
        IF(POLID.NE.'C')THEN
            ...
            IF(ISTAT.NE.0)THEN
                CONTXT='ERROR: PASS_DIR keyword missing from flat'//
     *                  ' field file'
                GO TO 999
            ENDIF
            IPOS=PASS1
            IF((PASS1.LT.1).OR.(PASS1.GT.2))THEN
                CONTXT='ERROR: invalid PASS_DIR in flat field file'
                GO TO 999
            ENDIF
        ENDIF
C
...
C
     ...
     *                  KEYWRD//NAME
        ...
        ENDIF
C
C Read flat field values
C
        CALL UIGL1R(IDIN,FF(1,IPOS),ISTAT)
        IF(ISTAT.NE.0)THEN
            CONTXT='ERROR reading flat field file '//NAME
            GO TO 998
        ENDIF
