        SUBROUTINE SPCPOL(FRAME,ROOT,PFLAGS,NFRAME,RETFIL,
     *                  CCS4,PCPFIL,WAVE,DATA,ERR,EPS,BAD,ISTAT)
*
*  Module number:
*
*  Module name: SPCPOL
...
*
*  FORTRAN name: spcpol.for
*
*  Keywords of accessed files and tables:
*  --------------------------------------
*  Name                         I/O     Description / Comments
*       retfil                  I       Retardation file
*       pcpfil                  I       Post-COSTAR polarimetry corrections file
...
*  -------------------
*  CDBS:
*       yrdret, yrccs4, ypolar, ymsput, ywshft, ypol2, ypol3
*
*  SDAS:
*	uhdgsd
*  Others:
...
*	1	Sep 93	H. Bushouse	Modified version of YCLPOL
*	2	Jun 94	H. Bushouse	Mods to handle NREAD > 1
*       3       Feb 98  M. De La Pena   Major mods to incorporate the post-
*                                       COSTAR corrections.
*       3.1     Apr 99  M. De La Pena   Check for post-COSTAR POLSCAN=4 data.
...
C same waveplate position). Added Jun 94 by H.A.B.
C
            NWVPOS = NFRAME / NREAD
            WVPOS = 0
...
C
            IF(NOBJ.LT.1)THEN
                CONTXT='ERROR: Polarimetry processing specfied with '//
...
C
            ...
            CALL YMSPUT(CONTXT,STDOUT,0,ISTAT)
C
C Read PA_APER from .C1H file header
C
c           CALL UHDGSD(IDS(1),'PA_APER',PGAPER,ISTAT)
            CALL UHDGSD(IDS(12),'PA_APER',PGAPER,ISTAT)
            IF(ISTAT.NE.0)THEN
                 CONTXT='ERROR getting PA_APER from .c1h file'
                 GO TO 999
            ENDIF
...
C
C Read POLANG from .C1H file header
C
c           CALL UHDGSR(IDS(1),'POLANG',POLANG,ISTAT)
            CALL UHDGSR(IDS(12),'POLANG',POLANG,ISTAT)
            IF(ISTAT.NE.0)THEN
     ...
     $        	        ' Value of 0 assumed.'
        	    ...
        	    POLANG = 0.0
        	ELSE 
                    CONTXT='ERROR getting POLANG from .c1h file'
                    GO TO 999
                ENDIF
            ENDIF
...
C pass 1
C
c           CALL YPOLAR(NFRAME,NX,1,FTAB(1,1,1),ETAB(1,1,1),
...
C
     ...
     *                     'correction for Pass 1.'
            ...
            DO 400 J=1,NUPDAT
                GROUP = J
c               CALL YCLWRT(ROOT,GROUP,PFLAGS,RES1(1,J),15,'POL',
                CALL SPCWRT(ROOT,GROUP,PFLAGS,TMPRES(1,J),15,'POL',
     *                          ISTAT)
                IF(ISTAT.NE.0)GO TO 1000
400         CONTINUE
C
...
C
            IF(NPASS.GT.1)THEN
c               CALL YPOLAR(NFRAME,NX,2,FTAB(1,1,2),ETAB(1,1,2),
...
C
     ...
     *                         ' correction for Pass 2.'
                ...
                DO 500 J=1,NUPDAT
                    GROUP = 14 + J
c                   CALL YCLWRT(ROOT,GROUP,PFLAGS,RES2(1,J),15,'POL',
                    CALL SPCWRT(ROOT,GROUP,PFLAGS,TMPRES(1,J),15,'POL',
     *                          ISTAT)
                    IF(ISTAT.NE.0)GO TO 1000
500             CONTINUE
...
C
     ...
     *                                   STDOUT,0,ISTAT)
                DO 600 J=1,NUPDAT
                    GROUP = 28 + J
c                   CALL YCLWRT(ROOT,GROUP,PFLAGS,RES1(1,J),15,'POL',
                    CALL SPCWRT(ROOT,GROUP,PFLAGS,RES1(1,J),15,'POL',
     *                          ISTAT)
                    IF(ISTAT.NE.0)GO TO 1000
600             CONTINUE
C
