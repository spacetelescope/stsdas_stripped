        SUBROUTINE ZCLMDF(IN,NS,WIDTH,OUT,ISTAT)
*
*  Module number:
*
*  Module name: zclmdf
...
*  Keyphrase:
*  ----------
*       Median filter the background
*
*  Description:
*  ------------
*       This routine performs a median filter of the background
*       with a user specified filter width.  The edge points
*       are filled with the value of the closest position that
*       can be filtered.
*
*  FORTRAN name: zclmdf.for
*
...
C
        IF((WIDTH.LT.0).OR.(WIDTH.GT.511))THEN
                CALL ZMSPUT('ERROR: invalid median filter width',
...
C                                    --->Center of the filter
                DO 100 I=1,N
C
C copy data into an array to sort and sort it
C
                        DO 20 J=1,WIDTH
20                              TEMP(J)=IN(J+IOFF)
                        CALL ZCLSRT(WIDTH,TEMP)
                        OUT(IPOS)=TEMP(CENTER)
C                                    --->take center point
                        IPOS=IPOS+1
                        IOFF=IOFF+1
100             CONTINUE
