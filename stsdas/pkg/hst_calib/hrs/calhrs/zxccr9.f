        SUBROUTINE ZXCCR9(CCR9,GRAT,ORDER,ID,
...
*  Keyphrase:
*  ----------
*       Index table CCR9 (GHRS echelle ripple interpolation coef.)
*
*  Description:
*  ------------
*       This routine reads table CCR9 and extracts the carrousel positions
*       and there row numbers for the specified grating, and order.
*       A sorted list of carrousel positions and row index is returned.
*  FORTRAN name: ZXCCR9.FOR
*
*  Keywords of accessed files and tables:
*  --------------------------------------
*  Name                         I/O     Description / Comments
*       CCR9                    I       Echelle ripple interp. coef. table
*
*  Subroutines Called:
*  -------------------
*  SDAS:
...
C LOCAL VARIABLES -------------------------------------------
C
        INTEGER NROWS,COLIDS(3),ROW,SPORD,N
	INTEGER ISTATS(2)
        CHARACTER*5 GMODE
        CHARACTER*80 CONTXT
        CHARACTER*15 COLNAM(3)
        LOGICAL NULL
        DATA COLNAM/'GRATING','SPORDER','CARPOS'/
C-------------------------------------------------------------------------
C
C Open table
C
        CALL UTTOPN(CCR9,RDONLY,ID,ISTAT)
        IF(ISTAT.NE.0)THEN
                CONTXT='ERROR opening CCR9 table '//CCR9
                GO TO 999
        ENDIF
C
...
C
        ...
        IF(ISTAT.NE.0)THEN
                CONTXT='ERROR locating needed columns in CCR9 table '//
     *                  CCR9
                GO TO 999
        ENDIF
C
C Locate rows with correct grating mode and order number
...
C
                ...
                IF(ISTAT.NE.0)GO TO 999
                IF(SPORD.EQ.ORDER)NFOUND=NFOUND+1
	    ENDIF
20	CONTINUE
C
...
C
        IF(NFOUND.EQ.0)THEN
                WRITE(CONTXT,199)GRAT,ORDER
199             FORMAT('ERROR - No rows found in CCR9 for grating  ',
     *                                A5,'  order',I3)
                GO TO 999
        ENDIF
C
