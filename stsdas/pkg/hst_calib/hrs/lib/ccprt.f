...
*
        DOUBLE PRECISION WAVE(1),DIODE(1),DELTAW(1),DELTAD(1)
        DOUBLE PRECISION TOTAL1(1),TOTAL2(1)
        INTEGER N,ISTAT
C
C LOCAL VARIABLES
C
...
C PRINT TITLES
C
        CALL UMSPUT(TITLE1,STDOUT,0,ISTAT)
        IF(ISTAT.NE.0)GO TO 999
        CALL UMSPUT(TITLE2,STDOUT,0,ISTAT)
        IF(ISTAT.NE.0)GO TO 999
C
C LOOP ON BINS
C
...
99              FORMAT(F10.2,F8.2,F10.2,F10.2,E14.4,E12.4)
                CALL UMSPUT(MESS,STDOUT,0,ISTAT)
100     CONTINUE
C
C DONE
C
