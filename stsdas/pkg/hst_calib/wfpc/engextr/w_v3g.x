...
#  w_v3g -- extract wfpc engineering data listed in Table V-3g
#
#  Description:
#  ------------
#  
...
#------------------------------------------------------------------------------
procedure w_v3g (indata, msb, lsb, inmask, tp, colidn, errval, nrows, 
			loc)
...
begin
	...
	call eng_write ("W01K049A", blank, blank, hexval1, nrows+1, tp, colidn)
	call eng_write ("W01K050A", blank, blank, hexval2, nrows+2, tp, colidn)
	nrows = nrows + 2

	# filter wheel counts [mnemonic 90, 92]
	call strcpy ("W01U090A",	mnemonic, SZ_MNEMONIC)
	indx1[1] = 88
	indx2[1] = 90
	indx1[2] = 90
	indx2[2] = 92
	...
	if (inmask[92] == OKVAL)
	    ...
	    hexval = errval
	call eng_write ("W01U094A", "CHKSUM", blank, hexval, nrows+1, tp, 
			colidn)
	nrows = nrows + 1
	...
	#else
	#    hexval = errval
	#call eng_write ("W01K095A", "EXECPORT", blank, hexval, nrows+1, tp, 
	#		colidn)
	#nrows = nrows + 1
	#call w_cmd (EXEC, msb, lsb, tp, colidn, nrows, loc)

	if (inmask[loc[INFREQ]] == OKVAL)
	    hexval = indata[loc[INFREQ]]
	else
	    hexval = errval
	call eng_write ("W01K097A", "INFQPORT", blank, hexval, nrows+1, tp, 
			colidn)
	nrows = nrows + 1
	call w_cmd (INFREQ, msb, lsb, tp, colidn, nrows, loc)

	if (inmask[loc[PREP]] == OKVAL)
	    hexval = indata[loc[PREP]]
	else
	    hexval = errval
	call eng_write ("W01K099A", "READOUTP", blank, hexval, nrows+1, tp, 
			colidn)
	nrows = nrows + 1
	call w_cmd (PREP, msb, lsb, tp, colidn, nrows, loc)

	if (inmask[loc[RIUA]] == OKVAL)
	    hexval = indata[loc[RIUA]]
	else
	    hexval = errval
	call eng_write ("W10K101A", "LASTRIUA", blank, hexval, nrows+1, tp, 
			colidn)
	nrows = nrows + 1
	...
	if (inmask[loc[RIUB]] == OKVAL)
	    ...
	    hexval = errval
	call eng_write ("W10K103A", "LASTRIUB", blank, hexval, nrows+1, tp, 
			colidn)
	nrows = nrows + 1
	...
	call strcpy ("W10J106A",	mnemonic, SZ_MNEMONIC)
	...
	call eng_write ("W01J109B", "WORK6144", chval, hexval, nrows+1, 
				tp, colidn)
	nrows = nrows + 1

	# Science ADC Data to mp, SDF Selected, Line Start Received,
	# RIU Eng. Data Received [mnemonic 110-114]
	call strcpy ("W01X110B",	mnemonic, SZ_MNEMONIC)
	...
	do k = 2, 1, -1 {
	    if (k == 1) {
	        call strcpy ("W10J120A",	mnemonic, SZ_MNEMONIC)
	        call strcpy ("RIUCMIDA",	keyword, SZ_KEYWORD)
	    } else {
	        call strcpy ("W10J119A",	mnemonic, SZ_MNEMONIC)
	...
	if (inmask[108] == OKVAL)
	    ...
	    hexval = errval
	call eng_write ("W01J129A", "ENGRADC", blank, hexval, nrows+1, tp, 
			colidn)
	nrows = nrows + 1

	# pyramid status [mnemonic 132]
	if (inmask[110] == OKVAL) {
	    hexval = bitson (msb[110], 7, 6)
	    if (hexval == 1)
	    	call strcpy ("PC",		chval, SZ_CHVAL)
	    else if (hexval == 2)
	    	call strcpy ("WF",		chval, SZ_CHVAL)
	    else 
	    	call strcpy ("ERR",		chval, SZ_CHVAL)
	} else {
	    hexval = errval
	    chval[1] = EOS
	}
	call eng_write ("W05X132B", "PYRAMID", chval, hexval, nrows+1, 
				tp, colidn)
	nrows = nrows + 1
	...
	call eng_write ("W11X134B", "APERTURE", chval, hexval, nrows+1, 
				tp, colidn)
	nrows = nrows + 1

	# blade sensors [mnemonic 135, 136]
	call strcpy ("W06X136B",	mnemonic, SZ_MNEMONIC)
	...
	call strcpy ("W03X137B",	mnemonic, SZ_MNEMONIC)
	...
	if (inmask[114] == OKVAL)
	    ...
	    hexval = errval
	call eng_write ("W01J169A", "SCIADCLN", blank, hexval, nrows+1, tp, 
			colidn)
	nrows = nrows + 1

	# shutter interpolations and speeds [mnemonic 170-181]
	call strcpy ("W06W170A",	mnemonic, SZ_MNEMONIC)
	...
	if (inmask[130] == OKVAL)
	    ...
	    hexval = errval
	call eng_write ("W06J186A", blank, blank, hexval, nrows+1, tp, colidn)
	nrows = nrows + 1
