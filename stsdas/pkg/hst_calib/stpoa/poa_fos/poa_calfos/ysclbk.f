...
*     /////////////////////////////////////////////////////////////////////
*     1.2       Jun 01  A. Alexov       Adding in POA common block for use of
*                                       new background scaling calc using the
*                                       SHELL parameter (LSHP)
*                                       Added SPECPHOTO DEACCUM background calc
*-------------------------------------------------------------------------------
*
* INPUTS:
*	frame - frame number
...
*	is - slice number
*	iy - ystep number
*       grndmd - mode of fos data
*
* INPUT/OUTPUT:
*	sback - scaled background spectrum
*	spec_sback - scaled background spectrum (spec mode, for output)
*
* OUTPUT:
*       istat - error status
*
...
C LOCAL VARIABLES ------------------------------------------------------
	...
	DOUBLE PRECISION SUM,MEAN,SCALEF,TEMP,SCALEPOA
...
C
C Calculate the POA background scale:
C fit of average dark count rate against shell parameter (LSHP), 
C for the dark calibration reference data sets
C
	IF (MEAN.GT.1.0D-7)THEN
            SCALEPOA=(0.007123*LSHP-0.002258)/MEAN
	    SCALEF=INTRATE/MEAN
	ELSE
	    SCALEF=1.
            SCALEPOA=1.
...
C output c7h spectrum is simply SCALEPOA
	    DO 122 I=1,NX
	       C7H_SBACK(I)=RBACK(I)*SCALEPOA
 122        CONTINUE

CCC            WRITE(CONTXT, 902), SCALEF
CCC 902        FORMAT('  DARKS: Original CALFOS SCALEF=' F14.7)
CCC            CALL YMSPUT(CONTXT,STDOUT+STDERR,0,ISTAT)
CCC
CCC            WRITE(CONTXT, 903), SCALEPOA, LSHP
CCC 903        FORMAT('  DARKS: SCALEPOA=' F14.7, ', SHELL=', F14.7)
CCC            CALL YMSPUT(CONTXT,STDOUT+STDERR,0,ISTAT)

C if SPECPHOTO data, need to accomodate non-deaccumed data array;
C therefore, adapt the poa back scale according to the previous
C value (if there was one)  
        IF (GRNDMD(1:12).NE.'SPECTROSCOPY') THEN
	   DO 120 I=1,NX
CCC	       SBACK(I)=RBACK(I)*SCALEF
	       SBACK(I)=RBACK(I)*SCALEPOA
120	   CONTINUE
        ELSE
C if SPECTROSCOPY, must use DEACCUM version of the POA DARK SCALE
C check if in first spectrum
           IF (CURRENT_GPNUM.EQ.0) THEN
              CURRENT_GPNUM=1
              PREV_SCALE=0.0
           ENDIF
           SCALEPOA=(SCALEPOA + (CURRENT_GPNUM - 1)*(PREV_SCALE))/
     $               CURRENT_GPNUM
           PREV_SCALE=SCALEPOA
CCC           WRITE(CONTXT, 904), SCALEPOA, CURRENT_GPNUM
CCC 904       FORMAT('  DARKS PER GROUP: SCALEPOA=' F14.7, ', GP=', I4)
CCC           CALL YMSPUT(CONTXT,STDOUT+STDERR,0,ISTAT)
	   DO 121 I=1,NX
	       SBACK(I)=RBACK(I)*SCALEPOA
121        CONTINUE
           CURRENT_GPNUM=CURRENT_GPNUM+1
