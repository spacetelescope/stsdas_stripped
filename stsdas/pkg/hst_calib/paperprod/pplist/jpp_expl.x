# Create the explanation page for the ACS paper product 

procedure jpp_expl (fd)
...
begin
	...
	yoff = yoff + 13
        ...
        call pp_move (fd, x1, yoff)
        call fprintf (fd, 
		"label '%sfBOptional Parameters Summary'\n")
            call pargstr ("\\")
	yoff = yoff + 2
	    ...
	    call pargr (small)
	call pp_label (fd, x2, yoff, 
	  "The Optional Parameters Summary contains additional instrument")
	call pp_label (fd, x2, yoff+1, 
	  "configuration information.  Entries in the table reflext the values of")
	call pp_label (fd, x2, yoff+2, 
	  "the Optional Parameters, other than pattern related, as specified in")
	call pp_label (fd, x2, yoff+3, 
	  "the observing proposal.")
	yoff = yoff + 4

    #
    # Start of next column of explanations
	#
    yoff = 3.5
