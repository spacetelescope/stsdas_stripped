...
char	fsci[SZ_FNAME]  	# Science file
...
begin
	...
	# get input parameters
        call clgstr ("rootname", root, SZ_FNAME)
        call clgstr ("fsci", fsci, SZ_FNAME)
	...
	call strcpy (root, trl, SZ_FNAME)
        ...
        call strcat ("[0]", fsci, SZ_FNAME)

        # open the input image
        im = immap (fsci, READ_ONLY, 0)
	...
	if (access(jih, 0, 0) == YES) {
	    ...
	    #opp_jitter (jih, script, 0.7, 0.95, 0.15, 0.5)
	} else {
	...
	y = 3.
        ...
        call sprintf (text, SZ_LINE, "CALSTIS Version: %s")
            call pargstr (str)
