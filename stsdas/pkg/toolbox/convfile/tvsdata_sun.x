...
# Phil Hodge, 25-Jul-1990  Create Sun version.
...
# Copy table data from VAX to Sun format.
...
begin
	...
	do row = 1, nrows {
		...
		case TY_DOUBLE:
		    ...
		    if ((inbufi[1] == TVS_VAX_INDEFD1) &&
			(inbufi[2] == TVS_VAX_INDEFD2))		# ***
			inbufd = INDEFD
		    else
			call v2sd (inbufd)			# ***
...
# Copy the data portion of a row-ordered table from Sun format to VAX format.
...
# Copy the data portion of a column-ordered table from Sun format to VAX format.
