...
define  STF_DEFHDREXTN  "hhh"		# Default extension for SDAS files
define  STF_HDRPATTERN  "^??h"		# SDAS extension pattern
define  HDR_EXTENSIONS  "|^imh|^hhh|^hhd|^??h|^??d|"
define  MAX_LENEXTN	3		# Maximum extension length
define  ENV_DEFIMTYPE	"imtype"	# CL environment variable

define	MSG_NOP		0		# No output (for UMSPUT)
define  MSG_MAXPRIO	10		# Max priority for output
define  IMHDR_PAR	0		# Image header parameter (for uhdas)
define  GROUP_PAR  	1		# Specific group parameter (for uhdas)

# Graphics parameters in addition to GIO definitions in <gset.h>

define	GR_STEP		2		# Step (pseudo-histogram) style curve
define	GR_BAR		3		# Bar graph (histogram)
...
# Characters to number and number to characters error code (Utility routines)
