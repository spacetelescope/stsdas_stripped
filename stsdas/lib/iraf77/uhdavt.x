...
# UHDAVT -- Add a new keyword array of the string type. A comment string
# can be associated with the keyword. It is not an error if the parameter 
# already exists.

procedure uhdavt (im, keyw, felem, nelem, sval, comm, htype, ier)
	...
	iferr {
	   for (i = felem; i < felem+nelem; i=i+1) {
	      ...
	      if (imaccf (im, Memc[kp]) == NO) {
	          call f77upk (sval[i], Memc[vp], SZ_VALSTR)
		  lenv = strlen (Memc[vp])
	          call imadcf (im, Memc[kp], lenv, TY_CHAR, Memc[cp], htype)
