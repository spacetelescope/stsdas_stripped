...
procedure uhdass (im, keyw, sval, comm, htype, ier)
...
begin
	...
	if (imaccf (im, Memc[kp]) == NO)
	   iferr (call imadcf (im, Memc[kp], lenv, TY_SHORT, Memc[cp], htype)) {
	       ier = errcode()
               if (ier == SYS_IDBOVFL)
	         ier = ER_HDRNOSP
	       else 
	...
	iferr (call imputs (im, Memc[kp], sval)) 
	              ier = ER_HDRPARTY
	else {
	   ier = ER_OK
	   IM_UPDATE(im) = YES
