...
# Phil Hodge, 12-Apr-2012  Character-string columns were being defined with
#	a length of 1, and TSCALi and TZEROi keywords were (erroneously) being
#	created for them.
...
int	dtype		# apparent data type, negative for a char-string column
...
begin
	...
	# First fill in the arrays of column names, etc.
	ttype0 = 0
	tform0 = 0
	tunit0 = 0
	do i = 1, nfields {
	    cp = tbcnum (tp, i)
	    dtype = COL_DTYPE(cp)		# negative for char column
		...
		call sprintf (Memc[tform+tform0], SZ_FTFORM, "%d%c%d")
		    call pargi (-dtype * nelem) # CFITSIO special convention
		    call pargc (dtype_c)
		    call pargi (-dtype)
	...
	do i = 1, nfields {
	    ...
	    # A character string column does not have a NULL value or TSCAL.
	    if (COL_DTYPE(cp) < 0)
		next
